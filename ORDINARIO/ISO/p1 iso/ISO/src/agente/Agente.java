...
public class Agente {
    ...
    //Implementación del patrón Singleton
     ...
     public boolean login() {
         ...
         try{
                 Agente.conectar();
                 PreparedStatement sentence = mBD.prepareStatement("SELECT login, passw FROM Cliente");
                 ResultSet respuesta = sentence.executeQuery();
                 
             ResultSetMetaData rsmd = respuesta.getMetaData();
             numColumnas = rsmd.getColumnCount();
             
                 //Vector basedatos = Agente.select("SELECT Login, Password FROM Usuarios");
    ...
    public Vector select(String SQL) throws SQLException,Exception{
    	...
    	while(respuesta.next()){
    		...
    		//Muy importante, empezamos a contar desde la posici—n 1, si buscamos
    		//en la posici—n 0 de los registros del ResultSet dar‡ un error
    		for(int i=1; i<=numColumnas; i++){
    			//En este switch controlamos el tipo de cada uno de las columnas del
    			//registro, y segœn el tipo, utilizamos un mŽtodo y otro de la clase
    			//ResultSet para extraer el dato.
    			//Para simplificar, podemos poner s—lo los tipos de dato que tenemos
    		...
    		//Al terminar de procesar cada registro, lo almacenamos en el vector resultado
    		resultado.add(registro);
    	}
    	//Al terminar, ya hemos procesado todos los registros del ResultSet, los tenemos
    	//en el vector, y podemos cerrar todos los punteros que hay hacia la BD y
